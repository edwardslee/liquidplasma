...
model{
  ...
  beta1_bar ~ normal(0, 10);
  beta2_bar ~ normal(0, 10);
  z1 ~ normal(0, 1);
  z2 ~ normal(0, 1);
  for (i in 1:78) {
    mu[i] = alpha + 
  ...
  fibrinogen ~ student_t(2, mu, sigma);
}
generated quantities{
  ...
  for (i in 1:78) log_lik[i] = student_t_lpdf(fibrinogen[i] | 2, mu[i], sigma);
}
